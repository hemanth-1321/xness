        ...
        async function createCandle(interval: string, viewName: string, schedule: string) {
            await client.query(`
                    ...
                    SELECT
                        ...
                        symbol,
                        first(price::NUMERIC / ${PRICE_MULTIPLIER}, time) AS open,
                        MAX(price::NUMERIC / ${PRICE_MULTIPLIER}) AS high,
                        MIN(price::NUMERIC / ${PRICE_MULTIPLIER}) AS low,
                        LAST(price::NUMERIC / ${PRICE_MULTIPLIER}, time) AS close,
                        SUM(volume::NUMERIC / ${VOLUME_MULTIPLIER}) AS volume
                    FROM trades
